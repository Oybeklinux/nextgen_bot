...
@dp.callback_query_handler(language_cb.filter(), state="select_language")
async def back_to_course(callback: types.CallbackQuery, state: FSMContext) -> None:
    ...
    await state.set_state("send_phone")


@dp.message_handler(content_types=[ContentType.CONTACT], state="send_phone")
